...
export default class Article {
    ...
    parseWithNewParser(elements) {
        ...
        const parsed = parseContent(doc, {
            pretitleLevel: 2,         // H2 before H1 (legacy behavior)
            parseCodeAsJson: true,    // Properties feature
            extractBodyHeadings: true // Body headings extraction
        });
    ...
    processNode(node, givenProfile = null) {
        ...
        if (processed.content) {
            processed.content = Array.isArray(processed.content)
                ? processed.content.map((child) => this.processNode(child, givenProfile))
